...
function accuracyBlock(): string[] {
  return [
    ...
    "",
    "If a search was attempted for this turn and returned nothing usable, you will be told so. Disclose that in ONE short sentence naming what you attempted and that it failed, and then immediately answer the question as well as you can from training knowledge, with a clear caveat that your information ends at your knowledge cutoff and may be out of date. The disclosure is a prefix, never the answer itself: a reply that stops at the disclosure leaves the user with nothing, which is worse than a stale answer honestly labelled. NEVER present remembered information as live, and NEVER claim the web had nothing when the search itself is what failed.",
    "",
...
function toolsBlock(): string[] {
  return [
    ...
    "",
    "A tool result is the ONLY source for what a tool produced. If a tool fails, times out, or returns an error, say plainly that it failed and what you could not determine, in one or two sentences rather than as the whole reply, and then still answer the user's question as well as you can from what you know, with that part clearly labelled unverified. A reply that is nothing but the failure notice leaves the user worse off than a stale answer honestly caveated.",
    "",
